...
PATH_IMAGENET = "/home/kirin/DATA/tfrecord"


class MNIST_datastream:
    def __init__(self, sess, batchsize=100, augmentation=False):
...
class ImageNet_datastream:
    ...
    def get_train_batch(self):
        ...
        # return image, label
        return tf.cast(self.train_img_batch, tf.float32), self.train_label_batch

    def get_test_batch(self):
        # image, label = self.sess.run([self.val_img_batch, self.val_label_batch])
        # return image, label
        return tf.cast(self.val_img_batch, tf.float32), self.val_label_batch
